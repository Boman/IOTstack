...
def main():
  ...
  documentationHint = 'https://sensorsiot.github.io/IOTstack/Containers/OctoPrint'
  ...
  def onResize(sig, action):
    global octoprintBuildOptions
    global currentMenuItemIndex
    mainRender(1, octoprintBuildOptions, currentMenuItemIndex)

  octoprintBuildOptions = []
  ...
  def mainRender(needsRender, menu, selection):
    ...
    if needsRender == 1:
      ...
      print(term.black_on_cornsilk4(term.center('IOTstack OctoPrint Options')))
      print("")
  ...
  def runSelection(selection):
    import types
    global octoprintBuildOptions
    if len(octoprintBuildOptions[selection]) > 1 and isinstance(octoprintBuildOptions[selection][1], types.FunctionType):
      octoprintBuildOptions[selection][1]()
    else:
  ...
  def menuEntryPoint():
    ...
    with term.fullscreen():
      menuNavigateDirection = 0
      mainRender(needsRender, octoprintBuildOptions, currentMenuItemIndex)
      selectionInProgress = True
      with term.cbreak():
        while selectionInProgress:
          ...
          if menuNavigateDirection != 0: # If a direction was pressed, find next selectable item
            ...
            currentMenuItemIndex = currentMenuItemIndex % len(octoprintBuildOptions)
            needsRender = 2

            while not isMenuItemSelectable(octoprintBuildOptions, currentMenuItemIndex):
              currentMenuItemIndex += menuNavigateDirection
              currentMenuItemIndex = currentMenuItemIndex % len(octoprintBuildOptions)
    return True
...
global currentServiceName # Name of the current service
if currentServiceName == 'octoprint':
  main()
else:
  print("Error. '{}' Tried to run 'octoprint' config".format(currentServiceName))
