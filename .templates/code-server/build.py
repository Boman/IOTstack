...
def main():
  ...
  documentationHint = 'https://sensorsiot.github.io/IOTstack/Containers/Code-server'
  ...
  def onResize(sig, action):
    ...
    global currentMenuItemIndex
    mainRender(1, codeServerBuildOptions, currentMenuItemIndex)
  ...
  def runSelection(selection):
    import types
    global codeServerBuildOptions
    if len(codeServerBuildOptions[selection]) > 1 and isinstance(codeServerBuildOptions[selection][1], types.FunctionType):
      codeServerBuildOptions[selection][1]()
    else:
  ...
  def menuEntryPoint():
    ...
    with term.fullscreen():
      menuNavigateDirection = 0
      mainRender(needsRender, codeServerBuildOptions, currentMenuItemIndex)
      selectionInProgress = True
      with term.cbreak():
        while selectionInProgress:
          ...
          if menuNavigateDirection != 0: # If a direction was pressed, find next selectable item
            ...
            currentMenuItemIndex = currentMenuItemIndex % len(codeServerBuildOptions)
            needsRender = 2

            while not isMenuItemSelectable(codeServerBuildOptions, currentMenuItemIndex):
              currentMenuItemIndex += menuNavigateDirection
              currentMenuItemIndex = currentMenuItemIndex % len(codeServerBuildOptions)
    return True
...
global currentServiceName # Name of the current service
if currentServiceName == 'code-server':
  main()
else:
  print("Error. '{}' Tried to run 'code-server' config".format(currentServiceName))
